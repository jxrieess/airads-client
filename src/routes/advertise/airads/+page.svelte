<script lang="ts">
	...
	let previewMessage = $derived(
		adMessage.trim() ||
			'Enjoy ₱10 discount! Present this coupon at any Julie’s Bakeshop branch.'
	);
	...
	<div class="panel">
		<div class="panel-grid">
			<div class="panel-main">
				...
				<input class="input" type="text" bind:value={goal} />

				<label class="label">Industry and Company Name</label>
				<input class="input" type="text" bind:value={industryCompany} readonly />

				<h3 class="section-title">Ad Creative</h3>
				<label class="label">Ad Message</label>
				<div class="message-wrap">
					<textarea
						class="textarea"
						bind:value={adMessage}
						maxlength={MAX_MESSAGE_LENGTH}
						rows="3"
						placeholder="Ad Message (Maximum of 80 characters, including spaces, punctuation, and line breaks)"
					/>
					<button type="button" class="clear-btn" on:click={clearAdMessage}>×</button>
				</div>
				...
				<div class="media-row">
					...
					<button type="button" class="media-card media-card--add" on:click={openFilePicker}>
						...
						/>
						{#if imagePreviewUrl}
							<img src={imagePreviewUrl} alt="Selected media" class="media-preview-img" />
						{:else}
							<span class="media-add-icon">+</span>
							<span class="media-add-text">Add media</span>
						{/if}
					</button>
				</div>
			</div>
